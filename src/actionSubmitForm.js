...
export async function actionSubmitForm(_, formData) {
  const data = {
  ...
  if (data.name.length === 1) errors.name = "Does your name really have only one character?";
  ...
  if (existingData.some(entry => entry.email === data.email)) {
    return {
      success: false, 
      errors: {email: "Email is already entered" }, 
