...
const Camping = ({ isOpen, setIsOpen, cartItems }) => {
  ...
  //       const ticketTotal = cartItems.reduce((sum, item) => sum+ item.price * item.count, 0)
      ...
      <ul className="mb-6 space-y-2">
        {campingData.length > 0
          ? campingData
              ...
              .map((spot, index) => (
                //<li {spot.available > ticketAmount ? "bg-gray-70 " : "p-4 border rounded-lg bg-gray-50 text-gray-700 flex flex-col"}
                <li
                  key={index}
                  className={
