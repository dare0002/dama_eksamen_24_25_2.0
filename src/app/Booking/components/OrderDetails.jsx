...
const OrderDetails = ({ cartItems, onUpdate }) => {
    ...
    useEffect(() => {
        const calculateTotal = () => {
            const itemsTotal = cartItems.reduce((sum, item) => sum+ item.price * item.count, 0);
    ...
    return (
        <div className="bg-offwhite bg-gray-100 p-6 shadow-md w-96">
            ...
            <div className="space-y-4">
                {cartItems.map((item) => (
                    <div key={item.id} className="flex justify-between items-center text-black">
                        <span className="text-sm flex items-center gap-2">
                            <BsTrash3 onClick={() => handleRemoveItem(item.id)} className="cursor-pointer"
                            /> 
                            {item.title} x {item.count}
                        </span>
                        <span className="font-medium">{(item.price * item.count).toFixed(2)} DKK</span>
                    </div>
                ))}
