...
const Checkout = () => {
    ...
    const handleCartUpdate = (updatedItem) => {
        setCartItems((prev) => {
            const existingItem = prev.find((item) => item.title === updatedItem.title);
            if (existingItem) {
    ...
    return (
        <section className="mb-12">
            <h3 className="text-xl md:text-2xl mb-6 w-full max-w-4xl mx-auto">Checkout.</h3>
            <div className="w-full max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="col-span-2 ">
                    <Accordion onCartUpdate={handleCartUpdate}/>
                </div>
